    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        <?php if (empty($menus)): ?>
            <div class="text-gray-500">
                <p>Tidak ada menu yang ditemukan untuk rating ini.</p>
            </div>
        <?php else: ?>
                ...
                <div class="flex items-center w-full p-10 overflow-hidden bg-white border-b border-gray-200 py-4">
                    ...
                    <div class="flex flex-col justify-between w-full">
                        <a href="<?= base_url('menus/' . $menu->id) ?>" class="mb-2">
                            ...
                            <h5 class="text-base  tracking-tight text-slate-500"><?= $menu->name_stores ?? "Menu Pertama" ?></h5>
                        </a>
                        <div class="flex items-center mb-1">
                            <?php for ($i = 1; $i <= 4; $i++): ?>
                                <?php if ($i <= $menu->rounded_rating): ?>
                                ...
                                <?php endif ?>
                            <?php endfor ?>
                            <span class="ml-2 rounded bg-yellow-200 px-2 py-0.5 text-xs font-semibold"><?= $menu->rounded_rating ?></span>
                        </div>
